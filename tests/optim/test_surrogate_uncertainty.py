...
def test_surrogate_uncertainty_execution():
    ...
    preds = surrogate.rank_candidates(
        candidates, minimize_objective=True, exploration_ratio=1.0
    )
    ...
    preds_no_exp = surrogate.rank_candidates(
        candidates, minimize_objective=True, exploration_ratio=0.0
    )
    score_no_exp = preds_no_exp[0].expected_value

    # Run with exploration_ratio=10.0
    preds_exp = surrogate.rank_candidates(
        candidates, minimize_objective=True, exploration_ratio=100.0
    )
