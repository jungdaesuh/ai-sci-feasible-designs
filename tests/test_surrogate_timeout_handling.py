...
class TestSurrogateTimeoutHandling(unittest.TestCase):
    def test_fit_timeout_handling(self):
        ...
        metrics = [{"metrics": {"vacuum_well": 0.1, "qi": 1.0, "max_elongation": 2.0}, "feasibility": 1.0, "candidate_params": {"r_cos": [[1.0]], "z_sin": [[1.0]]}}]
        targets = [1.0]
    ...
    def test_rank_timeout_handling(self):
        ...
        metrics = ([{"metrics": {"vacuum_well": 0.1, "qi": 1.0, "max_elongation": 2.0}, "feasibility": 1.0, "candidate_params": {"r_cos": [[1.0]], "z_sin": [[1.0]]}}] * 5 + 
                   [{"metrics": {"vacuum_well": -0.1, "qi": 2.0, "max_elongation": 3.0}, "feasibility": 0.0, "candidate_params": {"r_cos": [[1.0]], "z_sin": [[1.0]]}}] * 5)
        targets = [1.0] * 5 + [0.0] * 5
        bundle.fit(metrics, targets, minimize_objective=False)
