import torch
import pytest
from ai_scientist.optim.equivariance import random_rotation_matrix, quaternion_to_matrix
...
def test_random_rotation_matrix_properties(batch_size, device):
    ...
    identity_batch = torch.eye(3, device=torch.device(device)).unsqueeze(0).repeat(batch_size, 1, 1)
    ...
    assert torch.allclose(det_values, torch.ones(batch_size, device=torch.device(device)), atol=1e-6)
