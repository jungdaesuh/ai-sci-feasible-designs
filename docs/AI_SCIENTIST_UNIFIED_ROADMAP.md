...
- Add `_safe_evaluate(compute, stage, maximize=False)`: direction-aware penalties (+1e9 for minimize, -1e9 for maximize), recursive NaN/Inf clamp, cache penalized failures.
- Implement `compute_constraint_margins(metrics, problem)` returning signed violations; store margins + `max_violation` in results.
- Persist schema and hash: `structured_flatten(params, schema)`, where schema stores `(mpol, ntor, schema_version)` and rounding; hashes used in caches/DB.
- Tests: NaN produces penalty; minimize/maximize flags correct; flatten keeps R_cos(1,0) index stable across mpol/ntor.
...
- Hash canonicalization (round 1e-6) before insert; schema_version stored per record.
- Helpers: add/query feasible, near-feasible by `l2` margin, latest archive, cache stats.
- Tests: read/write roundtrip, resume from DB keeps archive/HV identical on stub model.
...
- Training policy: fit if ≥8 samples; if <8 log “cold start” and preserve input order; for regressors use feasible-only if ≥4 feasible else all.
- Ranking score: `E[value] = P(feasible) * corrected_objective`, with sign inverted for minimize to keep higher=better.
- Timeout guard on fit/predict; per-cycle retrain when K new points or every N cycles.
...
- Ranking: by feasibility prob − constraint-distance with small uncertainty bonus; keep exploration ratio from config.
- Evaluate top-K at `stage="screen"`; failures → infeasible with max violation; log per-cycle feasible count + HV if feasible exists.
- Retrain surrogate per policy; record cycle summaries in `cycles` table and `reports/cycle_<n>.json`.

### 5) HV / Objective Phase
- For P2: maximize `L∇B` with feasibility prob ≥ threshold.
- For P3: maintain Pareto archive on (Aspect Ratio, L∇B); compute HV each cycle; optimizer NSGA-II or NGOpt; promote top-K from surrogate batch to `refine/final` based on feasibility then objective.
...
- Roles in agent mode: planning (retrieve_rag, make_boundary, evaluate_p3), literature (retrieve_rag, write_note), analysis (evaluate_p3, make_boundary). Gate every tool via `AgentGate.validate_tool_call` and log `role/tool/context_hash`.
- PropertyGraph: materialize per cycle from world-model + literature notes; snapshot to `reports/graphs/` and feed to planner prompt.
- Daemon (`scripts/daemon.py`): cycle loop with checkpoint `reports/checkpoints/cycle_<n>.json`, wall-clock guard, resume flag; sets `OMP_NUM_THREADS=1` in workers.

### 7) RAG Rebuild (RPF)
- Convert PDFs with `markitdown 2506.19583v1.pdf -o docs/papers/2506.19583v1.md` and same for 2511.02824v2.
- Build unified index over papers + MASTER plan + Production + Updated/Unified plans; path `ai_scientist/rag_index.db`.
- Tests: `tests/test_rag_indexing.py` asserts top-k hits from both papers.

### 8) Adapter Loop (Autonomy + RPF)
- `scripts/train_adapters.py`: read `reports/ai_scientist.sqlite` statements/preferences, build SFT/DPO dataset, train PEFT adapter, save to `reports/adapters/<tool>/<stage>/adapter.safetensors` + metadata.
- Loader in `adapter.py`: loads newest adapter when `AI_SCIENTIST_PEFT=1` and path set; logs adapter version in run metadata.
- Cadence: nightly/manual; keep last N adapters, queue file `reports/adapters/queue.jsonl`.
