...
Atul Agrawal Maria Pascu Marĳa Savtchouk
...
Figure 2: A plasma boundary is defined by the coefficients 𝑅𝑚𝑛 and 𝑍𝑚𝑛 of a truncated Fourier
...
transformed into Boozer angles 𝜃 𝐵 and 𝜙𝐵, respectively, to straighten the magnetic field lines (black).
...
Assuming stellarator symmetry, 𝑅𝑚,𝑛 = 0; 𝑚 = 0, 𝑛 < 0 and 𝑍𝑚,𝑛 = 0; 𝑚 = 0, 𝑛 ≤ 0, and fixing the
...
only optimized configurations. Black lines represent the identity. 𝐴, 𝜖𝑚𝑎𝑥, and Δ𝑒𝑑𝑔𝑒 were used as
...
ΣΘ : (𝜃, 𝜑)
↦−→ (𝑅, 𝜙, 𝑍), where 𝜃 and 𝜑 are generic poloidal and toroidal angles, respectively,
...
where 𝑓𝑖 : R𝐷 → R are objective functions, 𝑐𝑖 : R𝐷 → R are constraint functions, and 𝑐∗
𝑖 are
...
maximum elongation 𝜖𝑚𝑎𝑥 for
a given aspect ratio 𝐴, edge
rotational transform ˜𝜄, and av-
erage triangularity ¯𝛿.
¯𝛿 aver-
...
Table 2 introduces the problem definition, where (cid:101)𝐿 ∇B is magnetic field gradient scale length [39]
normalized by 𝑎/𝑁fp, QI = 1
𝑄𝐼 d𝜃 d𝜙 quantifies deviation from a precise QI field follow-
4 𝜋2
...
∇𝛼) ∥∇𝑟 ∥2
2 as a simple geometric proxy. Here H is the Heavyside step function, B × 𝜅 · ∇𝛼 is
...
considering a QP magnetic field where the magnetic field strength forms a single well, i.e., 𝐵(𝜑) = 𝐵0 cos(𝑁fp𝜑),
...
[0, 1] interval (higher is better), ˜𝑐𝑖 = (𝑐𝑖 − 𝑐∗
𝑖 is the 𝑖-th normalized constraint violation, and 𝜀 is
a relative tolerance.

𝑖 )/𝑐∗

Multi-objective scoring For multi-objective problems, we compute the hypervolume (HV) indica-
tor [52, 53] over feasible solutions (i.e., those with ˜𝑐𝑖 (Θ) ≤ 𝜀, ∀𝑖 ) using a fixed reference point in
objectives space.
...
converting one objective into a constraint 𝐴 ≤ 𝐴∗ with
𝐴∗ ∈ {6, 8, 10, 12}. All instances were run on a 96-vCPU
machine for 15–24 h.

8

510˜L∇B681012A0100000200000functioncalls1020˜L∇B0100000200000functioncalls7.510.012.5A5 Generative modeling of feasible domains without access to the oracle
...
proximately 158𝑘 QI-like stellarator
...
E (Σ) = Equilibrium(cid:0)Ψ, Σ, 𝑀, 𝑁 (cid:1)
...
𝐽 𝜄 (Σ) = 𝜄 [Esol],
JO (Σ) = O (cid:2)Esol, O∗(cid:3),

𝑓 𝐴 = 𝑤 𝐴 (cid:0)𝐽𝐴 − 𝐴∗(cid:1) 2,

𝑓𝐸 = 𝑤𝐸 (cid:0)𝐽𝐸 − 𝐸 ∗(cid:1) 2,

𝑓 𝜄 = 𝑤 𝜄 (cid:0)𝐽 𝜄 − 𝜄∗(cid:1) 2,
(cid:13)
𝑓O = 𝑤 O
(cid:13)JO
...
omnigenous residual JO is computed by the DESC Omnigenity objective using the target field O∗.
...
𝑤(𝜂𝑖) (cid:2)𝐵eq(𝜌0, 𝜂𝑖, 𝛼 𝑗 ) − 𝐵∗ (𝜌0, 𝜂𝑖, 𝛼 𝑗 )(cid:3) 2,
...
(cid:0)so 𝑤 ≡ 1 if 𝜂weight = 1(cid:1),

where 𝐵eq is the field strength of the equilibrium in (𝜌, 𝜂, 𝛼) coordinates and 𝐵∗ is the perfectly-
omnigenous target field generated by OmnigenousField as in [8]. The residuals 𝑟𝑖 𝑗 = √︁𝑤(𝜂𝑖) (cid:0)𝐵eq −
𝐵∗(cid:1)

𝑖 𝑗 are evaluated on the same (𝜂, 𝛼)-grid used by the target field.

On a solved equilibrium Esol at a fixed flux surface 𝜌 = 𝜌0, we assemble a least-squares objective
L (Σ) = 𝑓 𝐴 + 𝑓𝐸 + 𝑓 𝜄 + 𝑓O .
...
(cid:101)Θ := diag(Λ) −1Θ and ˜𝑓 ((cid:101)Θ) := 𝑓 (diag(Λ)(cid:101)Θ) and ˜𝑐𝑖 ((cid:101)Θ) := 𝑐𝑖 (diag(Λ)(cid:101)Θ). In addition, we apply a
base-10 logarithmic transformation to the QI constraint.
In each iteration, the algorithm alternates between primal and dual updates. For each constraint ˜𝑐𝑖,
it tracks a penalty parameter 𝜌𝑘
𝑖 . The complete algorithm is given
in Algorithm 1.

𝑖 and a Lagrange multiplier 𝑦𝑘
...
+ , 𝛿0 > 0, 0 < 𝜏, 𝛾 < 1, 𝜎 > 1 and 𝛿min, 𝜌max > 0

(cid:101)Θ𝑘+1 = arg min
(cid:101)Θ∈ 𝐵((cid:101)Θ𝑘 , 𝛿𝑘 )
...
if ˜𝑐𝑖 ((cid:101)Θ𝑘+1) ≤ 𝜏 ˜𝑐𝑖 ((cid:101)Θ𝑘)
otherwise.
...
For the geometric problem we choose 𝜌0
𝑖 = 10, 𝜌max = 1𝑒9, 𝛿0 = 0.5, 𝛾 = 0.9, 𝛿min = 0.05, 𝜏 =
0.8, 𝜎 = 5. The subproblem (8) is solved with NGOpt with a budget of min{20.000, 1500 + 𝑘 · 260}
forward-model calls.
For the simple-to-build problem we choose 𝜌0
𝑖 = 10, 𝜌max = 1𝑒9, 𝛿0 = 0.5, 𝛾 = 0.95, 𝛿min = 0.05, 𝜏 =
0.8, 𝜎 = 5. The subproblem (8) is solved with NGOpt with a budget of min{20.000, 1500 + 𝑘 · 260}
forward-model calls.
For the MHD-stable problems we choose 𝜌0
𝑖 = 10, 𝜌max = 1𝑒8, 𝛿0 = 0.33, 𝛾 = 0.95, 𝛿min = 0.05, 𝜏 =
...
0.891.151.41|B|[T]0.60.81.01.2R [m]−0.4−0.20.00.20.4Z [m]'=0:002¼Nfp'=0:122¼Nfp'=0:252¼Nfp'=0:382¼Nfp'=0:502¼Nfp0.721.602.49|B|[T]0.60.81.01.21.41.6R [m]−0.20.00.20.40.6Z [m]'=0:002¼Nfp'=0:122¼Nfp'=0:252¼Nfp'=0:382¼Nfp'=0:502¼Nfp0.641.281.91|B|[T]0.91.01.1R [m]−0.10.00.10.20.3Z [m]'=0:002¼Nfp'=0:122¼Nfp'=0:252¼Nfp'=0:382¼Nfp'=0:502¼Nfp1.271.581.90|B|[T]0.80.91.01.1R [m]−0.2−0.10.00.10.20.3Z [m]'=0:002¼Nfp'=0:122¼Nfp'=0:252¼Nfp'=0:382¼Nfp'=0:502¼Nfp0.501.001.50|B|[T]0.951.001.051.10R [m]−0.10−0.050.000.050.100.150.20Z [m]'=0:002¼Nfp'=0:122¼Nfp'=0:252¼Nfp'=0:382¼Nfp'=0:502¼Nfp0.260.340.43|B|[T]0.80.91.01.1R [m]−0.2−0.10.00.10.20.30.4Z [m]'=0:002¼Nfp'=0:122¼Nfp'=0:252¼Nfp'=0:382¼Nfp'=0:502¼Nfp(a) geometric problem.
...
0200000functioncalls234˜L∇B0200000functioncalls0.10.20.3˜ι0200000functioncalls3.253.503.754.00A0200000functioncalls23(cid:15)max0200000functioncalls0.30.40.5∆−0.050.000.05functioncalls100101QI0200000functioncalls−0.3−0.2WMHD−0.050.000.05functioncalls−0.050.000.05hχ∇ri0200000functioncalls−0.4−0.20.0¯δ0250000functioncalls101520˜L∇B0250000functioncalls0.00.10.2˜ι0250000functioncalls91011A0250000functioncalls345(cid:15)max0250000functioncalls0.20.40.60.8∆0250000functioncalls10−410−3QI0250000functioncalls−0.10−0.05WMHD0250000functioncalls0.60.81.0hχ∇ri0250000functioncalls−0.10−0.050.000.05¯δ0200000functioncalls1020˜L∇B0200000functioncalls0.00.10.2˜ι0200000functioncalls7.510.012.5A0200000functioncalls51015(cid:15)max0200000functioncalls0.250.500.75∆0200000functioncalls10−310−2QI0200000functioncalls0.000.05WMHD0200000functioncalls12hχ∇ri0200000functioncalls0.00.5¯δ01000200030004000Iteration−400000−300000−200000−1000000Logprobability01000200030004000Iteration−35000−30000−25000−20000−15000−10000−50000LogprobabilityAlgorithm 2 Generative Inference of Feasible Configurations without Oracle Access
...
2: Project dataset to latent space: 𝑍 ← {z𝑖 = Φ(𝑥𝑖)} 𝑁
𝑖=1

// Feasibility Classification:

3: Train Random Forest classifiers {𝐶𝑖 (z)} 𝑁𝑐
4: Define soft-feasible region: ˜F ← (cid:209)𝑁𝑐
𝑖=1

𝑖=1 to predict feasibility label 𝑦 ∈ {0, 1}
{𝑝(𝐶𝑖 (z) = 1) ≥ 𝜏}, where 𝜏𝑖 = 0.8 ∀𝑖

// Density Estimation:

5: Fit Gaussian Mixture Model GMM(z) on data restricted to ˜F

// Bayesian Refinement:

6: Define prior: 𝑝(z) ← GMM(z)
7: Define quasi-likelihood: ℓ(z) ← (cid:205)𝑁𝑐
