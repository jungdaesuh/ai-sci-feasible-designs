...
def _run_cycle(
    ...
    *,
    ...
    coordinator = Coordinator(
        active_cfg, 
        world_model, 
        planner=ai_planner.PlanningAgent(), # Pass the planner instance (loads default model config)
    ...
    if active_cfg.aso.enabled:
        ...
        candidates = candidate_pool  # Assign to candidates for downstream processing
    ...
    if candidates:
        ...
        screen_results = []
    budget_controller.consume(len(screen_results))
    ...
    budget_controller.consume(len(promote_results))
    ...
    budget_controller.adjust_for_cycle(
        hv_delta=hv_delta,
