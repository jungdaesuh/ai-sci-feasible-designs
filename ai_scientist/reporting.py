    ...
    if isinstance(data_plane, Mapping):
        lines.extend(
            [
                ...
                f"  - Missing novelty metadata: {data_plane.get('novelty_missing_count', 0)}",
                f"  - Avg novelty: {data_plane.get('avg_novelty')}",
                f"  - Novelty reject threshold: {data_plane.get('novelty_reject_threshold')}",
                f"  - Novelty reject count: {data_plane.get('novelty_reject_count', 0)}",
                f"  - Novelty reject rate: {data_plane.get('novelty_reject_rate')}",
                f"  - Static path usage: {data_plane.get('static_path_rows', 0)}",
                f"  - Adaptive path usage: {data_plane.get('adaptive_path_rows', 0)}",
                f"  - Fallback static delegate usage: {data_plane.get('fallback_static_delegate_rows', 0)}",
